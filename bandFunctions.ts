interface Member {
  ...
  interface NewBandData {
    members: Members;
    all: string[];
    plays: Plays;
  }
  
  function transformBandData(band: { members: Members }): NewBandData {
    const allMembers = [...band.members.current, ...band.members.past];
  
    // lowercase and sort all members by age (DESC) and name (ASC)
    const allMembersLowerCase = allMembers.map((member) =>
      member.name.toLowerCase()
    );
    allMembersLowerCase.sort((a, b) => {
      const memberA: Member | undefined = allMembers.find((member) =>
        member.name.toLowerCase().includes(a)
      );
      const memberB = allMembers.find((member) =>
        member.name.toLowerCase().includes(b)
      );
      if (!memberA || !memberB) {
        return 0;
      }
